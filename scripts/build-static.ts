...
// netlify.toml already proxies /assets.json and /assets/<hash>/<file> on every
// domain this site serves, CATALOG_HOST included — those need nothing here.
// What it cannot express is a rule scoped to one hostname, which is exactly
// what the root-hash shape needs:
//
//   assets.warthog.network/<hash>/logo.png -> <backend>/assets/<hash>/logo.png
//
...
if (CATALOG_HOST) {
  ...
  writeFileSync(
    ...
    [
      ...
      `# ${CATALOG_HOST}: asset hash at the root, no /assets prefix.`,
      "# Same-origin /assets.json and /assets/* live in netlify.toml.",
      // A data host should answer with the catalog, not the submission form.
      `https://${CATALOG_HOST}/  ${catalogUrl}/assets.json  200!`,
      ...files.map(
        (f) => `https://${CATALOG_HOST}/:hash/${f}  ${catalogUrl}/assets/:hash/${f}  200!`,
      ),
      "",
    ].join("\n"),
  );
